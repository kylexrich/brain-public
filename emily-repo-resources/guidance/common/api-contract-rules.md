---
title: "Common API Contract Rules"
description: "common testing policy, Zod source-of-truth, schema registration, OpenAPI generator inputs, endpoint shape, request/response design, naming, nullability, pagination, errors, idempotency, generated artifacts, schema exports, schema composition, and testing policy."
order: 1
---

## **[STRICT] Testing Policy**

- Do not add or keep automated tests of any kind in `common/`: unit, integration, E2E, contract, snapshot, or generated-output tests.
- Do not add test runners, test configs, test scripts, fixtures, mocks, or scaffolding in `common/`.
- Validate `common/` changes through schema generation, type-checking, build, and the repo `$ci` (`/ci`) skill.
...
- **[STRICT]** Internal monorepo contract updates are applied in lockstep across `common/`, `app/`, and `client/` in the same commit; do not add backward-compatibility aliases or transitional endpoints unless explicitly requested.
- **[STRICT]** Public or external-facing interfaces require explicit compatibility, versioning, or migration handling instead of lockstep-only assumptions.
