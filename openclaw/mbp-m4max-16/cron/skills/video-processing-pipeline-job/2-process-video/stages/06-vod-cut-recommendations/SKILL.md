...
1. Read `openclaw/mbp-m4max-16/cron/skills/video-processing-pipeline-job/2-process-video/shared/worker-contract.md` for model, concurrency,
   and mode parameters.
2. Spawn one worker per chunk file, passing the fields below. Workers read chunk files directly from disk — never paste chunk content into
   ...
   | `cut_rubric_path`         | Absolute path to `openclaw/mbp-m4max-16/cron/skills/video-processing-pipeline-job/2-process-video/stages/06-vod-cut-recommendations/cut-rubric.md` |
