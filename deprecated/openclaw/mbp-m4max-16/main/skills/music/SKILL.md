...
| **Volume** | 08:00–18:00 → `20`. 18:00–21:00 → `15`. 21:00–23:00 → `8`. 23:00–08:00 → `4`. |
| **Mode** | Album → `repeat` (front-to-back, repeat all). Artist/playlist/vague → `shuffle` (shuffle + repeat all). |

## Play History

State file: `~/Developer/brain/deprecated/openclaw/mbp-m4max-16/main/state/music-history.json`
...
sonos-pr3 volume set --name "<Room>" <0-100>
sonos-pr3 group volume get --name "<Room>"            # group-wide volume
sonos-pr3 group volume set --name "<Room>" <0-100>    # set all grouped speakers at once
```
...
**Volume on grouped speakers after recovery:** `sonos-pr3 volume set --ip` only sets the coordinator's volume. Set volume on other grouped speakers via raw UPnP:

```bash
curl -s --connect-timeout 3 "http://<member-ip>:1400/MediaRenderer/RenderingControl/Control" \
  -X POST \
  -H "Content-Type: text/xml; charset=utf-8" \
  -H 'SOAPACTION: "urn:schemas-upnp-org:service:RenderingControl:1#SetVolume"' \
  -d '<?xml version="1.0" encoding="utf-8"?><s:Envelope xmlns:s="http://schemas.xmlsoap.org/soap/envelope/" s:encodingStyle="http://schemas.xmlsoap.org/encoding/"><s:Body><u:SetVolume xmlns:u="urn:schemas-upnp-org:service:RenderingControl:1"><InstanceID>0</InstanceID><Channel>Master</Channel><DesiredVolume><LEVEL></DesiredVolume></u:SetVolume></s:Body></s:Envelope>'
```
...
sonos-pr3 volume set --name "<Room>" <per defaults>    # only if nothing was already playing
