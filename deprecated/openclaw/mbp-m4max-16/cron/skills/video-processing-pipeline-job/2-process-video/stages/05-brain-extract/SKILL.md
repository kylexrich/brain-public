...
candidates with a single review agent (Opus, high thinking) before committing accepted artifacts to the vault with provenance backlinks.
...
1. Read `deprecated/openclaw/mbp-m4max-16/cron/skills/video-processing-pipeline-job/2-process-video/shared/worker-contract.md` and apply its worker
   parameters (model, concurrency, mode).
2. Spawn 1 worker per chunk file and pass: `chunk_file`, `chunk_file_path`, `extraction_types_dir_path`, and `candidates_dir`.
...
1. Spawn a single review agent (model: `anthropic/claude-opus-4-6`, thinking: `high`).
2. Read all candidate files from `<parent(output_file)>/candidates/`.
3. Read type rules from `extraction-types/` for each type encountered.
4. Read destination vault folder AGENTS.md rules for each target folder (`vault/beliefs/`, `vault/experiences/`, etc.).
5. Deduplicate candidates across chunks (same concept from different chunks = one candidate).
6. Quality-gate each remaining candidate against the criteria in `extraction-types/<type>.md`.
7. For accepted candidates, write vault artifacts as Markdown to the correct destination folder per type with a provenance backlink to the
   stream.
8. Write `output_file` conforming to `brain_extract.output.template.jsonc`, populating `reviewed_candidates` with one entry per
   reviewed candidate including its disposition (`created | updated | skipped`) and `vault_path`.
9. Return counts: `chunks_analyzed`, `raw_candidate_count`, `deduped_candidate_count`, `artifacts_created`, `artifacts_updated`,
   `artifacts_skipped`.
