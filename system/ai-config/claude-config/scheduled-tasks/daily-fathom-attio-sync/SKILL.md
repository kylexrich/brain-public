...
Steps:
1. Invoke the `sync-fathom-to-attio` skill (located at `~/Developer/brain/system/ai-config/claude-config/skills/sync-fathom-to-attio/SKILL.md` — read it before acting). The skill is scoped to a single Fathom account, so run it twice in sequence.
2. First run — inputs:
