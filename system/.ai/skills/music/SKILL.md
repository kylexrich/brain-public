...
4. **Taste-profile tiebreaker.** When search returns multiple plausible results with no history signal, use Kyle's known taste (indie, jazz, soul, R&B, classic rock, modern artists) to pick the most fitting one — never ask. A modern indie/jazz artist vs. a 19th-century classical composer is not genuine ambiguity; it's an obvious pick.

The goal: never ask when the answer is reasonably inferable. **Asking is a last resort, not a default.** Guessing wrong and being corrected is far preferable to interrupting Kyle with a clarification question.
...
curl -s --connect-timeout 3 "http://192.168.50.114:1400/ZoneGroupTopology/Control" \
  -X POST -H "Content-Type: text/xml; charset=utf-8" \
...
**If Stage 2 times out on 192.168.50.114 (Sonos):** try Stage 2 on another reachable IP to identify a working coordinator, then use `--ip <working-ip>` for all subsequent commands.

**If any speaker shows DOWN in Stage 1:** before declaring it offline, run `sonos-pr3 discover --format json` to check if its IP changed via DHCP. Speakers can silently get new IPs on lease renewal. If discover returns a different IP for a speaker, the speaker is online — update the IPs in the Speakers table, the Stage 1 loop, the Stage 2 URL, and the UUID table in Known Limitation #7, then update the music skill memory entry with the new IPs. Re-run Stage 1 with the corrected IPs before proceeding. Only report a speaker as truly offline if it does not appear in `sonos-pr3 discover` output at all.

**If all speakers are DOWN and not found by discover:** report to Kyle — nothing can be done.
...
Speaker IPs: Bathroom `192.168.50.236`, Bedroom `192.168.50.39`, Kitchen `192.168.50.244`, Living Room `192.168.50.114`.
...
WORKING_IP="192.168.50.114"  # example: Living Room
...
- **Resolve ambiguity with historical data, never by asking.** When a search returns multiple results (e.g. Vol. 1, Vol. 2, Vol. 3 of the same series), check recent-played, heavy-rotation, and music-history.json. If Kyle has listened to one of them before, that's the answer — play it without asking. Only ask when there's genuine ambiguity that history can't resolve (e.g. two completely different artists with the same name, neither in history). **Use Kyle's known taste profile to break ties when history is empty** — if one result fits the vibe of what he normally listens to (indie, jazz, soul, R&B, classic rock) and the other clearly doesn't, pick the fitting one. Example: "berlioz" → catalog returns a modern indie/jazz artist ("open this wall") and a classical composer — given Kyle's taste, the modern artist is the obvious pick; don't ask. Only ask when both candidates are genuinely plausible given his taste AND history gives no signal.
